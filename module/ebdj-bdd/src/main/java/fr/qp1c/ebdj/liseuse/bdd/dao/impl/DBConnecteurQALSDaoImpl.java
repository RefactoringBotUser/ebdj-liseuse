...
public class DBConnecteurQALSDaoImpl extends DBConnecteurGeneriqueImpl implements DBConnecteurQALSDao {
	...
	public Theme4ALS donnerTheme(int groupeCategorie, int niveau) {
		...
		String requete = String.format(
				"SELECT reference FROM THEME_QALS where reference NOT IN (SELECT DISTINCT reference FROM THEME_QALS_LECTURE) AND reference NOT IN (SELECT DISTINCT reference FROM THEME_QALS_PRESENTE) AND groupeCategorieRef=%d AND active=1 AND difficulte=%d;",
	...
	public boolean existerThemeNonJoue(int groupeCategorieRef, int niveau) {
		return compterNbQuestionNonLue("QALS",
				ajouterClauseGroupeCategorieRef(groupeCategorieRef) + " " + ajouterClauseDifficulte(niveau)) > 0;
	}
	...
	private void creerQuestions(Long reference, Map<Integer, Question4ALSBdjDistante> questions) {
		for (Entry<Integer, Question4ALSBdjDistante> question4ALS : questions.entrySet()) {
			String requete = String.format(
					"INSERT INTO QUESTION_QALS (seq,question,reponse,reference) VALUES ('%d','%s','%s',%d);",
					question4ALS.getKey(), DBUtils.escapeSql(question4ALS.getValue().getQuestion()),
					DBUtils.escapeSql(question4ALS.getValue().getReponse()), reference);
	...
	public void corrigerTheme(Theme4ALSBdjDistante theme4als) {
		// Création de la requête
		String requete = String.format(
				"UPDATE THEME_QALS SET difficulte=%d, theme='%s', club='%s', dateReception='%s', version=%d, categorie='%s', categorieRef=%d, groupeCategorieRef=%d WHERE reference=%d;",
				theme4als.getDifficulte(), DBUtils.escapeSql(theme4als.getTheme()),
				DBUtils.escapeSql(theme4als.getClub()), theme4als.getDateEnvoi(), theme4als.getVersion(),
				DBUtils.escapeSql(theme4als.getCategorie4ALS()), theme4als.getCategorie4ALSRef(),
				theme4als.getGroupeCategorie4ALS(), theme4als.getReference());
		executerUpdateOuInsert(requete);
		executerUpdateOuInsert(
				String.format("delete from QUESTION_QALS WHERE reference=%s;", theme4als.getReference()));
	...
	public void marquerThemePresente(String referenceTheme, String lecteur) {
		// Création de la requête
		String requete = String.format(
				"INSERT INTO THEME_QALS_PRESENTE (reference,date_presentation,lecteur) VALUES (%s,'%s','%s');",
				referenceTheme, Utils.recupererDateHeureCourante(), lecteur);
	...
	public Map<String, Map<String, Long>> compterParGroupeCategorieLue() {
		// Création de la requête
		String requete = "SELECT groupeCategorieRef, difficulte, count(difficulte) FROM THEME_QALS WHERE reference IN (SELECT DISTINCT reference FROM THEME_QALS_LECTURE) GROUP BY groupeCategorieRef, difficulte order by groupeCategorieRef;";
		return compterParGroupeCategorie(requete);
	}
