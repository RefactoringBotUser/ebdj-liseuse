...
public class DBConnecteurFAFDaoImpl extends DBConnecteurGeneriqueImpl implements DBConnecteurFAFDao {
	...
	public QuestionFAF donnerQuestionsJouable(List<Long> categoriesAExclure, Long niveauMin, Long niveauMax) {
		...
		String requete = String.format(
				"SELECT id,question,reponse,theme,difficulte,categorie,categorieRef,reference,version,club,dateReception FROM QUESTION_FAF Q_FAF WHERE Q_FAF.reference NOT IN (SELECT DISTINCT Q_FAF_J.reference FROM QUESTION_FAF_LECTURE Q_FAF_J) AND difficulte>=%d AND difficulte<=%d",
				niveauMin, niveauMax);
	...
	public QuestionFAF donnerQuestionsJouable(List<Long> categoriesAExclure, Long niveau) {
		// Création de la requête
		String requete = String.format(
				"SELECT id,question,reponse,theme,difficulte,categorie,categorieRef,reference,version,club,dateReception FROM QUESTION_FAF Q_FAF WHERE Q_FAF.reference NOT IN (SELECT DISTINCT Q_FAF_J.reference FROM QUESTION_FAF_LECTURE Q_FAF_J) AND difficulte=%d",
				niveau);
	...
	public Map<String, Long> compterParCategorieLue() {
		return compter(
				"SELECT categorie, count(1) FROM QUESTION_FAF Q_FAF WHERE Q_FAF.reference IN (SELECT DISTINCT Q_FAF_J.reference FROM QUESTION_FAF_LECTURE Q_FAF_J) GROUP BY categorie order by categorie;");
	}
	...
	public void corrigerQuestion(QuestionFAFBdjDistante questionFaf) {
		// Création de la requête
		String requete = String.format(
				"UPDATE QUESTION_FAF SET categorie='%s', categorieRef=%d, theme='%s', question='%s', reponse='%s', difficulte=%d, club='%s', dateReception='%s', version=%d WHERE reference=%d;",
				questionFaf.getCategorieFAF(), questionFaf.getCategorieFAFRef(),
				DBUtils.escapeSql(questionFaf.getTheme()), DBUtils.escapeSql(questionFaf.getQuestion()),
				DBUtils.escapeSql(questionFaf.getReponse()), questionFaf.getDifficulte(),
				DBUtils.escapeSql(questionFaf.getClub()), questionFaf.getDateEnvoi(), questionFaf.getVersion(),
				questionFaf.getReference());
